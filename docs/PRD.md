...
- ~~No real-time multi-user collaboration in v1.~~ Added after v1.0 as peer-to-peer live sessions (section 6.7). Still no collaboration server of our own.
...
### 6.7 Live collaboration (peer-to-peer)

Design agreed 2026-09-04. Sessions are WebRTC data channels between browsers, brokered by a PeerJS signalling server. The signalling server only exchanges connection offers and ICE candidates; diagram content never passes through it. Media (DTLS) encryption is built into WebRTC.

**Roles.** The person who starts a session is the **host**. Everyone who opens the session link is a **guest**. Topology is a star: guests connect to the host, the host relays edits between guests. The host is the single owner of the document's file identity.

| ID    | Requirement                                                                                                                                                                                                                                                                                                                      | Priority |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| LC-1  | "Share live" starts a session and produces a link of the form `#live:<sessionId>`. The session id is a random 24+ character string generated client-side; whoever has the link can join.                                                                                                                                         | Must     |
| LC-2  | Shared state is exactly: diagram source, diagram theme, and participant presence (name, colour, cursor/selection). Concurrent edits merge via a CRDT (Yjs) so no keystrokes are lost and no participant blocks another.                                                                                                          | Must     |
| LC-3  | **Never shared:** AI key, model, conversations and pending proposals; UI settings (layout, preview mode, UI theme); file origin, file name, local file handles, Drive identity or token; recent files. Each client keeps its own. An AI proposal becomes visible to others only once its author accepts it, as an ordinary edit. | Must     |
| LC-4  | Undo is per participant: undo reverts only your own changes (Yjs UndoManager), so one person cannot undo another's work. The shared CRDT update log is the session's change history.                                                                                                                                             | Must     |
| LC-5  | Guests see a session title (host-chosen, default "Shared diagram") instead of the host's file name, and a "Shared by <host>" badge. Their File menu offers only **Save a copy** (to their own device or their own Google Drive) and Export. Save-in-place, Drive conflict flows and the host's origin are invisible to them.     | Must     |
| LC-6  | Only the host can save to the original file or Drive location. Host saves are local to the host; guests are not notified beyond the ordinary presence UI.                                                                                                                                                                        | Must     |
| LC-7  | When the host ends the session (or disconnects for longer than 30 s), guests are told the session ended and keep the last synced content as an ordinary unsaved local document, with autosave and Undo intact.                                                                                                                   | Must     |
| LC-8  | Guests auto-reconnect for 30 s after a drop. The host's session id is kept in sessionStorage so a host reload resumes the same session and link.                                                                                                                                                                                 | Should   |
| LC-9  | Host can toggle "guests can edit" (default on). With it off, guests are read-only and see the change live.                                                                                                                                                                                                                       | Should   |
| LC-10 | Participant list with names and colours in the toolbar; remote cursors and selections in the editor. Names are chosen locally (default "Guest N"), stored in localStorage, never verified.                                                                                                                                       | Should   |
| LC-11 | While in a session the URL fragment sync (UR-1) pauses; the address bar shows the live link. The static share link (`#pako:`) remains available from the Share menu and encodes the current content as usual.                                                                                                                    | Must     |
| LC-12 | The signalling server host, port, path and ICE servers are build-time configurable (`VITE_PEER_*`). Default is the public PeerJS cloud server with Google STUN. No TURN server is bundled, so some peers behind strict NATs will fail to connect; the UI must say so plainly after the reconnect window.                         | Must     |
| LC-13 | Collaboration code (PeerJS, Yjs, y-codemirror) is a lazy chunk loaded only when starting or joining a session.                                                                                                                                                                                                                   | Must     |
| LC-14 | Joining a session is a fresh document for the guest. If the guest has unsaved work, the same Undo toast as for New/Open applies.                                                                                                                                                                                                 | Must     |
...
| PR-4 | No analytics, no third-party scripts beyond Google Identity/Picker and the bundled app. Live sessions add one third-party service: a PeerJS signalling server (default the public PeerJS cloud), which sees peer ids and connection metadata but no content. | Must     |
...
- **No native browser dialogs.** `alert`, `confirm` and `prompt` are not used. Names and destinations are collected in an in-app save panel; destructive replacements (new, open) proceed immediately and offer Undo in a toast. The only remaining browser dialogs are the ones the platform owns: the OS file picker and the `beforeunload` guard.
...
- **Security.** Strict Content Security Policy allowing scripts only from self and Google's identity/picker origins, and `connect-src` limited to self, Google APIs, OpenRouter and the configured PeerJS signalling server (WebSocket). No `eval`. Mermaid `securityLevel: 'strict'`.
...
- **Live sessions.** The session link is a bearer capability: anyone holding it can join and, unless the host turns editing off, edit. Links should be shared like a document link, and hosts should end sessions when done. The signalling server learns peer ids and IP-level connection metadata, as any WebRTC broker does; content travels only on the encrypted peer channel. Guests never receive the host's file identity, Drive token or AI data because those are never put on the shared document in the first place (LC-3), not merely hidden in the UI.
...
| WebRTC without TURN fails for peers behind symmetric NATs or strict corporate firewalls.                                                  | Some pairs cannot collaborate.                 | State it in the UI after the reconnect window; make ICE servers configurable so a team can add its own TURN; offer the static share link as the fallback. |
| The public PeerJS signalling server has no SLA and rate limits.                                                                           | Sessions cannot start.                         | Configurable server; document self-hosting `peer` in one command.                                                                                         |
| A guest with the link is indistinguishable from an invited one.                                                                           | Unwanted edits.                                | Read-only toggle, random 24+ char ids, end-session control. Join approval is a possible follow-up.                                                        |
